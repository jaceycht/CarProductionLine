...
int main(int argc, char** argv)
{
	...
	// prepare work_pack
	// no. of threads equals no. of workers
	...
	// Calculate the number of cars each work group has to make
	if (num_cars%(num_workers/8) == 0)
		count = num_cars/(num_workers/8);
	else 
		count = num_cars/(num_workers/8) + 1;

	// Loop for no. of cars (per work group)
	for (k=0; k < count; k++) {
		...
		// Use less workers if no. of cars to be made < no. of work groups needed
		int cars_left = num_cars - *sem_value;
		if (cars_left < (num_workers/8)) 
			NUM_THREADS = 8 * cars_left;

		// Loop for no. of workers
