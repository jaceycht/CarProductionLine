...
int requestSpace(sem_t *space) {
    ...
    int count=0;
	while(sem_trywait(space) == -1) { 
		if(count==2) {
			printf("No space left. Exit program.\n");
			exit(0);	// fail to find space and exit
 		}
 		sleep(6);	// to wait for someone to free the space
		count++;	// increment of count to let it try multiple times
